...
import { API_BASE, sendEmail } from "../services/api";
...
export const EmailComposer: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const finalHtml =
      htmlBody ||
      (body
        ? `<div style="font-family: Arial, sans-serif; font-size: 15px; line-height: 1.6; color: #202124;">${body.replace(/\n/g, "<br/>")}</div>`
        : "");
  ...
  return (
    <div className="glass-panel p-6 sm:p-8 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6 pb-6 border-b border-slate-800">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Visual WYSIWYG Email Document Editor with Automatic 1x1 Pixel Open
            Tracking.
          </p>
        </div>
      ...
      {statusResult && (
        <div
          className={`p-4 rounded-xl mb-6 border text-sm flex items-center justify-between ${statusResult.type === "success" ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-300" : "bg-rose-500/10 border-rose-500/30 text-rose-300"}`}
        >
        ...
        <div>
          <div className="flex items-center justify-between border-b border-slate-800 mb-3 pb-2">
            ...
            <div className="flex items-center gap-1 bg-slate-900 p-1 rounded-lg border border-slate-800 text-xs">
              <button
                type="button"
                onClick={() => setActiveBodyTab("editor")}
                className={`flex items-center gap-1 px-3 py-1 rounded-md transition-all ${activeBodyTab === "editor" ? "bg-indigo-600 text-white" : "text-slate-400 hover:text-slate-200"}`}
              >
                <FileText className="h-3.5 w-3.5" /> Visual Canvas
              </button>
              <button
                type="button"
                onClick={() => setActiveBodyTab("code")}
                className={`flex items-center gap-1 px-3 py-1 rounded-md transition-all ${activeBodyTab === "code" ? "bg-indigo-600 text-white" : "text-slate-400 hover:text-slate-200"}`}
              >
              ...
              <button
                ...
                className={`flex items-center gap-1 px-3 py-1 rounded-md transition-all ${activeBodyTab === "preview" ? "bg-indigo-600 text-white" : "text-slate-400 hover:text-slate-200"}`}
              >
          ...
          {activeBodyTab === "editor" && (
            <EditorToolbar execCmd={execCmd} insertSnippet={insertSnippet} />
          )}
          ...
          {activeBodyTab === "code" && (
            <textarea
              ...
              value={
                htmlBody ||
                (body
                  ? `<div style="font-family: Arial, sans-serif; font-size: 15px; color: #202124; line-height: 1.6;">${body.replace(/\n/g, "<br/>")}</div>`
                  : "")
              }
              onChange={(e) => {
        ...
        <div className="flex justify-end pt-4 border-t border-slate-800">
          ...
          >
            {sending ? (
              ...
              <>
                <Send className="h-4 w-4" /> Dispatch to {parsedEmails.length}{" "}
                Recipient{parsedEmails.length !== 1 ? "s" : ""}
              </>
            )}
          </button>
        </div>
