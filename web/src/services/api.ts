...
export async function fetchConfiguration(): Promise<APIResponse<ConfigResponse>> {
  const res = await fetch(`${API_BASE}/configuration`);
  if (!res.ok) throw new Error('Failed to fetch configuration');
  return res.json();
}

export async function updateConfiguration(config: ConfigUpdateRequest): Promise<APIResponse<ConfigResponse>> {
  const res = await fetch(`${API_BASE}/configuration`, {
    method: 'POST',
...
export async function sendEmail(payload: EmailMessage, isSync: boolean = false): Promise<APIResponse> {
  const endpoint = isSync ? `${API_BASE}/send-email/sync` : `${API_BASE}/send-email`;
